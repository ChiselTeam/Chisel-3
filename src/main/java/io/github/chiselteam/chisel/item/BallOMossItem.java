...
import io.github.chiselteam.chisel.entity.BallOMoss;
...
public class BallOMossItem extends Item implements ProjectileItem {
    public BallOMossItem(Properties properties) {
        super(properties);
    }
    ...
    public @NonNull InteractionResult use(Level level, Player player, @NonNull InteractionHand hand) {
        ...
        if (level instanceof ServerLevel serverLevel) {
            Projectile.spawnProjectileFromRotation(BallOMoss::new, serverLevel, itemStack, player, 0.0F, 1.5F, 1.0F);
        }
    ...
    public @NonNull Projectile asProjectile(@NonNull Level level, @NonNull Position position, @NonNull ItemStack itemStack, @NonNull Direction direction) {
        return new BallOMoss(level, position.x(), position.y(), position.z(), itemStack);
    }
    ...
    public void appendHoverText(@NonNull ItemStack stack, @NonNull TooltipContext context, @NonNull TooltipDisplay display, @NonNull Consumer<Component> builder, @NonNull TooltipFlag flag) {
        builder.accept(Component.translatable("item.chisel.ball_o_moss.desc").withStyle(ChatFormatting.GRAY));
    }
}
