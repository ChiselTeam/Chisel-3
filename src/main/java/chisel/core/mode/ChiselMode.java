...
public class ChiselMode {
    ...
    public static final Codec<ChiselMode> CODEC = Identifier.CODEC.xmap(id -> chisel.registry.ChiselModes.REGISTRY.getOptional(id).orElseGet(() -> new ChiselMode(id)), ChiselMode::registryName);
    public static final StreamCodec<io.netty.buffer.ByteBuf, ChiselMode> STREAM_CODEC = Identifier.STREAM_CODEC.map(id -> chisel.registry.ChiselModes.REGISTRY.getOptional(id).orElseGet(() -> new ChiselMode(id)), ChiselMode::registryName);
